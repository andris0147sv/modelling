...
if __name__ == '__main__':
    ...
    tempLeft = Sc[0] / numpy.sqrt(mu[0] * eps[0])
    koeffABCLeft = (tempLeft - 1) / (tempLeft + 1)

    tempRight = Sc[-1] / numpy.sqrt(mu[-1] * eps[-1])
    ...
    for t in range(maxTime):
        ...
        Hy[sourcePos - 1] -= (Sc[sourcePos - 1] / (W0 * mu[sourcePos - 1]) *
                              numpy.exp(-(t - 30.0) ** 2 / 100.0))

        # Граничные условия ABC в упрощенном виде
        # Ez[0] = Ez[1]
        # Ez[-1] = Ez[-2]
        ...
        Ez[sourcePos] += (Sc[sourcePos] / (numpy.sqrt(eps[sourcePos] * mu[sourcePos])) *
                          numpy.exp(-((t + 0.5) - (-0.5 * numpy.sqrt(eps[sourcePos] * mu[sourcePos]) / Sc[sourcePos]) - 30.0) ** 2 / 100.0))
